...
## Strumieniowanie dużych źródeł (generator + `ShouldBufferPayloads`)

`getChunks()` może być generatorem konsumowanym leniwie przez orchestrator —
zobacz `docs/BUFFERED_PAYLOADS.md` (dokładny mechanizm, co jest i nie jest
"streamowane") oraz `docs/use-cases/streaming-xml-import.md` i
`docs/use-cases/small-generator-no-buffering.md` (pełne przykłady, kiedy
wystarczy sam generator, a kiedy potrzebne jest `ShouldBufferPayloads`).
